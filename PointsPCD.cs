	...
	*/
	...
	IEnumerator loadOFF(string dPath){
		...
		for (int i = 0; i< numPoints; i++){
			...
			if (buffer.Length == 2){
				colors[i] = new Color ((System.Convert.ToInt32(double.Parse(buffer[3], CultureInfo.InvariantCulture)) >> 0) & 255,
									(System.Convert.ToInt32(double.Parse(buffer[3], CultureInfo.InvariantCulture)) >> 8) & 255,
									(System.Convert.ToInt32(double.Parse(buffer[3], CultureInfo.InvariantCulture)) >> 16) & 255);
			}else
		...
		try{ //incase of duplicates
			Destroy(GameObject.Find(filename)); // slow operation
		} catch (Exception e) {
